...
def object_plot(obj_x, obj_F1, obj_F2, *args, plot_mode='normal', mode='1', color1='', color2='', label_string=''):
    ...
    if plot_mode == 'normal':
        ...
        if mode == '1':
            ...
            args[0].plot(args[2], np.array(obj_F2) - min(obj_F2), label=label_string, c=color2, linestyle=next(linestyles), marker=next(markers), markersize=10, 
                         markevery=1)
    # For scatter plots
    elif plot_mode == 'scatter':
        args[0].plot(obj_x, obj_F1, label=label_string, c=color1[0], linestyle='-')
        args[1].scatter(args[2], obj_F2, label=label_string, edgecolors=color2[0], marker='o', facecolors='none')
    # For any other type of plot
    else:
        if color2 == '':
            color2 = next(colors)
...
def object_plot_doule(ax1, x1, y1, y2, ax2, x2, y3, y4, mode='1', color1=['r', 'b'], color2=['m', 'c'], label_string=''):
    if mode == '1':
        ...
        ax2.scatter(x2, y3, label=label_string, edgecolors=color2[0], marker='o', facecolors='none')
        ax2.scatter(x2, y4, label=label_string, edgecolors=color2[1], marker='o', facecolors='none')
    else:
