...
'''
import unittest
from ngeoFE_unittests.Mechanics.Cauchy.OneD.BVP.Cauchy1D_Von_Mises import Cauchy1DFEformulation, Cauchy1DFEproblem
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.my_FEproblem=Cauchy1DFEproblem(cls.my_FEformulation)
    ...
    def test_shear_force_displacement_values(self):
        ...
        with open(reference_data_path+"/Cauchy1D_Von_Mises_force_disp_values.out", "wb") as fp:   #Pickling
            pickle.dump(values, fp)
        
        #read data from binary files
        with open(reference_data_path+"/Cauchy1D_Von_Mises_force_disp_values.out", "rb") as fp:   #Pickling
        ...
        if equal:
            ...
            plotting_params.plot_legends(ngeo_parameters.reference_data_path, fig, filename='Cauchy_1D_Von_Mises_elastoplasticity_sigma_u',mode='1')
    ...
    def test_identical_elastoplastic_matrix(self):  
        ...
        #write data to binary files
        with open(reference_data_path+"/Cauchy1D_Von_Mises__elastoplastic_modulo.out", "wb") as fp:   #Pickling
            pickle.dump(values, fp)  
        

                #read data from binary files
        with open(reference_data_path+"/Cauchy1D_Von_Mises__elastoplastic_modulo.out", "rb") as fp:   #Pickling
            values_ref=pickle.load(fp)        
        values_diff=values_ref-values
