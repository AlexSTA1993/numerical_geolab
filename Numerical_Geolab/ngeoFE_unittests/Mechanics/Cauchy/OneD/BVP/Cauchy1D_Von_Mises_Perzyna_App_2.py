...
class Gauss_point_Querry(SubDomain):
    def inside(self, x, on_boundary):
        return x[0] >= 1./2.-1./80. #1/ny, ny: the number of elements
...
class Cauchy1DFEproblem(UserFEproblem):
    ...
    def history_svars_output(self):
        ...
        """
        hist_svars=[[1,[1,[21]]]] #Calculate elasto-viscoplastic shear strain increment
        return hist_svars    
    ...
    def plot_me(self):
        ...
        # font = {'size'   : 18}
        # plt.rc('font', **font)
        # nodal_force_disp=np.array(self.feobj.problem_history)
        # print(nodal_force_disp.shape())
        # # Pavg=FunctionSpace(self.mesh,"DG",0)
        # # p=plot(self.feobj.usol[0])#, label='$u_2$')
        # # #plt.colorbar(p);
        # p=plot(nodal_force_disp[:,1],nodal_force_disp[:,2])
        # plt.show()
        return
...
#         print('tmax', my_FEproblem.slv.tmax)
    ...
    def extract_force_disp(self):
        ...
        self.array_force=self.array_force[:].copy().sum(axis=1)/1.
