...
class Cauchy1DFEproblem(UserFEproblem):
    ...
    def create_mesh(self):
        ...
        h=self.h
        self.ny=100#80
        mesh = IntervalMesh(self.ny,-h/2.,h/2.)#"crossed")
    ...
    class Gauss_point_Querry(SubDomain):
        ...
        def inside(self, x, on_boundary):
            return between(x[0], (-1./2.*self.h, 1./2.*self.h))
    ...
    def history_svars_output(self):
        ...
        hist_svars=[
                    [1,[1,[21]]]
                    ] #Calculate elasto-viscoplastic shear strain increment
        return hist_svars    
    ...
    def set_materials(self):
        ...
        umat_id=1       # described a 2D material
    ...
    def run_analysis_procedure(self,reference_data_path):
        saveto=reference_data_path+"./Cauchy_1D_Von_Mises_Imperfection_step_0"+'_'+str(self.imp)+".xdmf"
        self.problem_step = 0
        ...
        for i in range(nsteps):
            ...
            saveto= reference_data_path+"./Cauchy_1D_Von_Mises_Imperfection_step_"+str(i+1)+'_'+str(self.imp)+".xdmf"
            converged=self.solve(saveto,summary=True)
    ...
    def extract_force_disp(self):
        ...
        self.array_force=self.array_force[:].copy().sum(axis=1)/1.#self.w
        self.array_force=self.array_force.reshape((-1,1))
        self.array_disp=self.array_disp[:,0].reshape((-1,1)).copy()
