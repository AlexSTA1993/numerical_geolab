...
import os
import unittest
from ngeoFE_unittests.Mechanics.Cauchy.ThreeD.BVP.Cauchy3D_Drucker_Prager_App_1_Isotropic_compression_initial import Cauchy3DFEformulation, Cauchy3DFEproblem
...
class Test(unittest.TestCase):
    ...
    def test_shear_force_displacement_values(self):
        '''
        Tests calculated nodal forces and displacements to values in ./reference_data/Cauchy3D_Drucker_Prager_force_disp_values.out
        '''
        ...
        # print('hello!!!')
        # print(os.path.abspath(reference_data_path+"Cauchy2D_Drucker_Prager_force_disp_values_App_1.out"))
        #

        with open(reference_data_path+"Cauchy3D_Drucker_Prager_force_disp_values_App_1.out", "wb") as fp:   #Pickling
            pickle.dump(values1,fp)        
        

        with open(reference_data_path+"Cauchy3D_Drucker_Prager_force_disp_values_App_1.out", "rb") as fp:   #Pickling
        ...
        with open(reference_data_path+"Cauchy3D_Drucker_Prager_svars_values_App_1.out", "wb") as fp:   #Pickling
            pickle.dump(svars_values,fp)        
    
        with open(reference_data_path+"Cauchy3D_Drucker_Prager_svars_values_App_1.out", "rb") as fp:   #Pickling
        ...
        if equal_1 and equal_2:
            ...
            sigma22= svars_values[:,6]
            sigma33= svars_values[:,12]
            sigma23= svars_values[:,18]
            sigma13= svars_values[:,24]
            sigma12= svars_values[:,30]
            #
            epsilon11= svars_values[:,36]
            epsilon22= svars_values[:,42]
            epsilon33= svars_values[:,48]
            epsilon23= svars_values[:,54]
            epsilon13= svars_values[:,60]
            epsilon12= svars_values[:,66]            
        ...
        #     y1.insert(0,0)
            ...
            plotting_params.plot_legends(ngeo_parameters.reference_data_path, fig, filename='Cauchy_3D_Drucker-Prager_App_1_J2_t',mode='1')
            ...
            plotting_params.show_plot()
            plotting_params.plot_legends(ngeo_parameters.reference_data_path, fig, filename='Cauchy_3D_Drucker-Prager_App_1_J2_q',mode='1')
            ...
            plotting_params.show_plot()
            plotting_params.plot_legends(ngeo_parameters.reference_data_path, fig, filename='Cauchy_3D_Drucker-Prager_App_1_J2_p',mode='1')
    ...
    #     with open(reference_data_path+"Cauchy2D_Drucker_Prager_elastoplastic_modulo_App_1.out", "wb") as fp:   #Pickling
    #         pickle.dump(values, fp)  
    #
    #     #read data from binary files
    #     with open(reference_data_path+"Cauchy2D_Drucker_Prager_elastoplastic_modulo_App_1.out", "rb") as fp:   #Pickling
    #         values_ref=pickle.load(fp)        
    ...
    def test_analytical__lambda_dot(self):  
        ...
        epsilon_p_11=svars_values[:,72]
        epsilon_p_22=svars_values[:,78]
        epsilon_p_33=svars_values[:,84]
        epsilon_p_23=svars_values[:,90]
        epsilon_p_13=svars_values[:,96]
        epsilon_p_12=svars_values[:,112]
