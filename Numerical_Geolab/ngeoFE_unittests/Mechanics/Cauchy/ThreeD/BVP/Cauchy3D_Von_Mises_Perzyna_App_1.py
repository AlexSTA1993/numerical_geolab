...
class Cauchy3DFEformulation(FEformulation):
    ...
    def generalized_epsilon(self,v):
        ...
        gde=[
            ...
            Dx(v[1],1),  #gamma_22
            Dx(v[2],2),  #gamma_33
            Dx(v[1],2)+Dx(v[2],1),  #gamma_12
            Dx(v[0],2)+Dx(v[2],0),  #gamma_12
...
class Gauss_point_Querry(SubDomain):
    def inside(self, x, on_boundary):
        return x[0] >= 1./2.-1./80. and between(x[1], (-0.1,0.1)) and between(x[2], (-0.1,0.1))
...
class Cauchy3DFEproblem(UserFEproblem):
    ...
    def give_me_solver_params(self,scale_t=1.):
            ...
            self.slv.dtmax=0.05*self.scale_t
    ...
    def run_analysis_procedure(self,reference_data_path):
        saveto=reference_data_path+"./Cauchy_3D_Von_Mises_Perzyna_App1_test_step_0.xdmf"
        self.problem_step = 0
        self.bcs=self.set_bcs()
        self.feobj.symbolic_bcs = sorted(self.bcs, key=itemgetter(1))
        print("initial")
        converged=self.solve(saveto,summary=True)
        scale_t_program = [self.scale_t,self.scale_t,self.scale_t,self.scale_t,self.scale_t,self.scale_t,self.scale_t,self.scale_t,self.scale_t]
        print("shearing1")
        ...
        for i in range(nsteps):
            ...
            saveto= reference_data_path+"./Cauchy_3D_s_Perzyna_App1_test_step_"+str(i+1)+".xdmf"
            converged=self.solve(saveto,summary=True)
