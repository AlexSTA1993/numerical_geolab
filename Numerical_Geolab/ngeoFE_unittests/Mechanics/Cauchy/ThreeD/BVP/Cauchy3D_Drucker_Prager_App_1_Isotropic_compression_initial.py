...
class Cauchy3DFEproblem(UserFEproblem):
    ...
    def __init__(self,FEformulation):
        ...
        self.ny=6 # element number along horizontal axes
        self.nw=6 # element number along vertical axes
        self.nz=6 # element number along vertical axes
    ...
    def create_mesh(self):
        ...
        nw=self.nw#10
        nz=self.nz#10
        mesh = BoxMesh(Point(-h/2.,-w/2.,-z/2.),Point(h/2.,w/2.,z/2.),ny,nw,nz)#"crossed")
    ...
    class top(SubDomain):
        def inside(self,x,on_boundary):
            return x[2] > 0.49  and on_boundary
    ...
    class Gauss_point_Querry(SubDomain):
        def __init__(self,z,nz,w,nw,h,ny):
            self.z=z
            self.nz=nz
            self.w=w
            self.nw=nw
        ...
        def inside(self, x, on_boundary):
            ...
            return between(x[0], (-self.w/(self.nw),self.w/(self.nw))) and between(x[1], (-self.h/(self.ny),self.h/(self.ny))) and between(x[2], (-self.z/(self.nz),self.z/(self.nz)))
    ...
    def create_Gauss_point_querry_domain(self,mesh):
        ...
        GaussDomainQuerry= self.Gauss_point_Querry(self.z,self.nz,self.w,self.nw,self.h,self.ny)
    ...
    def mark_boundaries(self,boundaries):
        ...
        top0=self.top()
        top0.mark(boundaries,6)
        bottom0=self.bottom()
        bottom0.mark(boundaries,5)
        back0=self.back()
        back0.mark(boundaries,3)
        front0=self.front()
        front0.mark(boundaries,4)
        return
    ...
    def set_bcs(self):
        ...
        if self.problem_step == 0:
            bcs=[
                ...
                # [1,[1,[0],-self.Normal_loading_eff]],
                # [2,[1,[0],self.Normal_loading_eff]],
                #
                # [3,[1,[1],-self.Normal_loading_eff]],
                # [4,[1,[1],self.Normal_loading_eff]],
                #
                # [5,[1,[2],-self.Normal_loading_eff]],
                # [6,[1,[2],self.Normal_loading_eff]],
                
                [[10,[-0.5,-0.5,-0.5]],[0,[0],0.]],#self.Normal_loading_total
                [[10,[-0.5,-0.5,-0.5]],[0,[1],0.]],
                [[10,[-0.5,-0.5,-0.5]],[0,[2],0.]],
                
                [[11,[0.5,-0.5,-0.5]],[0,[1],0.]],#self.Normal_loading_total
                [[11,[0.5,-0.5,-0.5]],[0,[2],0.]],
                
                [[12,[0.5,0.5,-0.5]],[0,[2],0.]],#self.Normal_loading_total

                [[13,[-0.5,0.5,-0.5]],[0,[0],0.]],#self.Normal_loading_total
                [[13,[-0.5,0.5,-0.5]],[0,[2],0.]],
                
                [[14,[0.5,-0.5,0.5]],[0,[1],0.]],#self.Normal_loading_total

                [[15,[-0.5,-0.5,0.5]],[0,[0],0.]],#self.Normal_loading_total
                [[15,[-0.5,-0.5,0.5]],[0,[1],0.]],
                
                [[16,[-0.5,0.5,0.5]],[0,[0],0.]],#self.Normal_loading_total
    ...
    def run_analysis_procedure(self,reference_data_path):
        saveto=reference_data_path+"Cauchy_3D_Drucker-Prager_test_step_0"+"_App_1"+".xdmf"
        self.problem_step = 0
...
#         print('tmax', my_FEproblem.slv.tmax)
            filename = 'Cauchy_3D_Drucker-Prager_test_step_'+str(i+1)
            saveto= reference_data_path+"Cauchy_3D_Drucker-Prager_test_step_"+str(i+1)+"_App_1"+".xdmf"
            converged=self.solve(saveto,summary=True)
