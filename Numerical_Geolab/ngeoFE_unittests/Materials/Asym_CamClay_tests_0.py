...
def set_material_1_properties(EE, nu, M, pc, pt, k, Hev, Ha):
    """
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        '''
        Run a load path for Asymmetric Cam Clay Material
        '''
        # Get the material from the library
        cls.notfirsttime = True
        env_lib = ngeo_parameters.env_lib
        umat_lib_path = ngeo_parameters.umat_lib_path
        umat_lib = umat_lib_path + 'CAUCHY3D-ASYMCAMCLAY/libplast_Cauchy3D_Asym_CamClay.so'
        umat_id = 1  # If many materials exist in the same library
        mat = UserMaterial(env_lib,umat_lib,umat_id)
        mat.props = set_material_1_properties(20., 0., 1.57, -5., -.7, 0.-.93, 0., 0.)

        # Set up the loading increments
        increments = 100
        e11min = 0.; e11max = -.75; deps11 = (e11max - e11min)/increments
    ...
    def test_stresses(self):
        ...
        reference_data = reference_data_path + 'Asym_CamClay_stress_values.out'   
        ...
        # Get the difference of the stresses, find the norm, and assert that it's small enough    
    ...
    def test_total_deformations(self):
        ...
        reference_data = reference_data_path + 'Asym_CamClay_epsilon_values.out'      
 ...
 #           pickle.dump(values, fp)
        ...
        # Get the difference of the deformations, find the norm, and assert that it's small enough
        values_diff = values_ref - values
