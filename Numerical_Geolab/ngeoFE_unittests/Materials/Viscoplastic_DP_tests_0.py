...
def set_material_1_properties(EE, nu, cc, tanfi, tanpsi, Hsfi, Hscc, rate_sensitivity):
    """
    Sets material parameters
    """
    GG = EE/(2.*(1. + nu))
    KK = EE*GG/(3.*(3.*GG - EE))
    props = np.array([KK, GG, 0., 0., 0., 0., 0., 0., 0., 0., tanfi, cc, tanpsi, Hsfi, Hscc, 0., 0., 0., rate_sensitivity])
...
class Test(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        mat.props = set_material_1_properties(20.e3, 0., 80., 0., 0., 0., -5., 8000.)
        # Set the loading
        increments = 100
    ...
    def test_stresses(self):
        '''
        Tests Viscoplastic Drucker-Prager material (stresses)
        '''
        # Create the stress values
        values = np.array(self.stress)
        # Get the reference values
        reference_data = reference_data_path + 'ViscoDP_stress_values.out'
        #with open(reference_data, "wb") as fp:  # Pickling
        #    pickle.dump(values, fp)
        with open(reference_data, "rb") as fp:  # Unpickling
            values_ref = pickle.load(fp)
        # Get the difference of the values and its norm
        values_diff = values_ref - values
        equal = abs(np.linalg.norm(values_diff)) <= 1.e-10
        # If the norm is too big, the test fails
        self.assertTrue(equal, "Not identical stresses: " + str(abs(np.linalg.norm(values_diff))))
        
    def test_total_deformations(self):
        '''
        Tests Viscoplastic Drucker-Prager material (deformations)
        '''
        # Create the deformation values
        values = np.array(self.epsilon)
        # Get the reference values
        reference_data = reference_data_path + 'ViscoDP_epsilon_values.out'
        #with open(reference_data, "wb") as fp:  # Pickling
        #    pickle.dump(values, fp)
        with open(reference_data, "rb") as fp:  # Unpickling
            values_ref = pickle.load(fp)
