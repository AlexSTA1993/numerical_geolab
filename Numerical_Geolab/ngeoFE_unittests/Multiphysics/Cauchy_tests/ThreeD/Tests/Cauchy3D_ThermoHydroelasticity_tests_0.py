...
class Test(unittest.TestCase):
    ...
    def test_generalized_force_displacement_values(self):
        ...
        with open(reference_data_path + "Cauchy3D_THM_ThermoHydroelasticity_0.out", "rb") as fp:  # Unpickling
            values_ref = pickle.load(fp)        
        values_diff = values_ref[:] - values1
        equal = abs(np.linalg.norm(values_diff)) <= 1.e-6
        # Asserts that the generated values match the stored values
        self.assertTrue(equal, "Not identical time, displacements, forces: " + str(abs(np.linalg.norm(values_diff)))) 
        # If everything is correct and plots are activated then plot diagram
        if equal and activate_plots:
            ...
            with open(filepath, 'rb') as f:
                ...
                analytical_temperature_values = np.reshape(np.fromfile(f, dtype=np.dtype('float64'), count=reduce(lambda x,y:x*y, dims)), dims)
            ...
            fig, ax1, ax2, color1, color2 = plotting_params.object_plot_axes(
                '$t$ [s]', 
                y1_txt='$T_{num}\; [^o $ C$]$', 
                color1='', 
                y2_txt='$T_{an}\; [^o $ C$]$', 
    ...
    def test_thermal_diffusion(self):  
        ...
        x_ind = np.where(np.in1d(time, xy))[0]
        y_ind = np.where(np.in1d(values_time1, xy))[0]
        values_diff = np.abs(analytical_temperature_values[x_ind, 1] - num_temperature_values[y_ind])
        equal = abs(np.linalg.norm(values_diff)) <= 1.e-4  # The error depends on the discretisation of the domain
        self.assertTrue(equal, "Not identical temperature evolution: " + str(abs(np.linalg.norm(values_diff))))    

        if equal and activate_plots:
            
            fig, ax1, color1 = plotting_params.object_plot_axes(
                '$t$ [s]', 
                y1_txt='$\|\\frac{\dot{T}_{an}-\dot{T}}{\dot{T}_{an}}\|$', 
                color1='k', 
                y2_txt='', 
                color2='', 
                title='', 
