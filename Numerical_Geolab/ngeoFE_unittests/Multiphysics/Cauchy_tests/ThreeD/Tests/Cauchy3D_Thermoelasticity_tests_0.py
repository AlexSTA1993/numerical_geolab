...
class Test(unittest.TestCase):
    ...
    def test_generalized_force_displacement_values(self):
        ...
        with open(reference_data_path + "Cauchy3D_THM_Thermoelasticity_0.out", "rb") as fp:  # Unpickling
            values_ref = pickle.load(fp)        
        values_diff = values_ref - values1
    
        equal = abs(np.linalg.norm(values_diff)) <= 1.e-8  # Docker precision is reduced however it is better than the accuracy of the equlibrium solver 10e-6 and material solver 10e-8
        # We assert that the reference values and the value produced in the simulation match
        self.assertTrue(equal, "Not identical time, displacements, forces: " + str(abs(np.linalg.norm(values_diff))))
        
        # If everything is correct then plot diagram
    ...
    def test_steady_state_displacement_values(self):
        """
        Test the values that result from the steady-state problem
        """
        filepath = reference_data_path + 'thermal_diffusion_analytical_results.txt'       
        analytical_temperature_values = np.loadtxt(filepath)

        DT = analytical_temperature_values[-1, 1]  # Applied tempererature at the unittest
        u0 = 1.*1.*DT  # Measured displacement at the top of the specimen along x0
        u1 = 1.*1.*DT  # Measured displacement at the top of the specimen along x1    
        u2 = 10.*1.*DT  # Measured displacement at the top of the specimen along x2    
