...
class Test(unittest.TestCase):
    ...
    def test_generalized_force_displacement_values(self):
        ...
        #with open(reference_data_path+"Cauchy3D_THM_Thermoelasticity.out", "wb") as fp:   #Pickling
        #    pickle.dump(values1,fp)        
        
        with open(reference_data_path+"Cauchy3D_THM_Thermoelasticity.out", "rb") as fp:   #Pickling
            values_ref=pickle.load(fp)        
        values_diff=values_ref-values1
    
        equal=abs(np.linalg.norm(values_diff))<=1.e-8 #docker precision is reduced it however better than the accuracy of the equlibrium solver 10e-6 and material solver 10e-8
