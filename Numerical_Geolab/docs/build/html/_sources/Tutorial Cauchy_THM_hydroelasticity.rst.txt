=================================================
Cauchy THM couplings: Hydroelasticity application
=================================================

In this tutorial we will validate the complex behavior of our model in elasticity in the presence of hydro-mechanical couplings.
Our model is a rectangular block of unit base dimensions and of length 10 mm. Initially the bottom left and back faces of the block are 
constrained along their normal dimensions. In the free facets of the block (top, right, front) a common total compressive Normal stress 
is applied, together with a uniform pore fluid pressure.

The boundaries of the cantilever are considered to be insulated from its surroundings meaning that no fluid flow is allowed to take place. Under this configuration, 
the fluid flow inside the block is reminiscent of the 1D Darcy flow. We will be checking the diffusion time, the pore fluid pressure evolution at the free end of the cantilever and the developped displacements at the end of the analysis, when steady state has been achieved. 

The file for this tutorial can be found in: 
:py:mod:`ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.Cauchy3D_DP_Hydroelasticity_tests_0`
...
We proceed now with building the main Finite element model, corresponding to a 3D Cauchy elastic block with a pore fluid pressure increase 
at its base. The finite element model is built inside a child class that inherits its methods from the parentclass UserFEproblem(). 
...
.. figure:: _images/3D_Hydroelasticity_block.png
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
...
:py:meth:`set_material_1_properties()<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_HM_Hydroelasticity_0.set_material_1_properties>`).
...
Our test takes place in a fluid saturated geomaterial, which lies under total compression :py:const:`Normal_loading total` =-200 MPa. 
The pore fluid pressure is set to be :py:const:`self.Pressure_loading` =66 MPa, thus initially the material lies under uniform effective 
copressive stress :py:const:`self.Normal_loading_eff` =-133.33 MPa according to the Terzaghi principle. We need to specify the initial state at the start of the analysis,
by making use of the function :py:meth:`set_initial_conditions()` :
...
The way we apply the boundary conditions can be seen in the method :py:meth:`set_bcs()` : 
...
In this example, the boundary conditions change at different steps of the analysis. At the begining o the analysis the initial conditions of the problem
are assigned to the block. The region id ranges from 1 to 6. In order to apply the correct value of the effective stress we apply the normal displacement constraint in the (bottom, left and back) surface of the block,
and the common total normal loading stress at the free facets. After successful application of the initial conditions, wev proceed at the main part of the analysis, which is the application
of the pore fluid pressure increase at the bottom facet of the block. We specify also that the temperature increase is zero. For the displacement components of the vectorspace ([0],[1],[2]), the type of bc is set to 0, 
specifying incremental Dirichlet boundary conditions. For the pore fluid pressure component of the vectorspace ([3]) we specify the value 2 for the type of the boundary condition
indicating an instantaneous Dirichlet boundary condition (pore fluid pressure increase) of value 10 MPa. For the temperature component of the vectorspace ([4]) the type of boundary condition is set to 0 indicating 
an incremental Dirichlet boundary condition of zero temperature. 
...
Next, we define the analysis material that we have already assigned in the :py:meth:`create_subdomains()` method.
...
end of the cantilever when steady state is achieved. To do so we will set the hydraulic permeability of the material at :py:const:`chi` =1 :math:`\frac{\text{mm}^2}{\text{MPa s}}` 
...
due to swelling we will set the hydraulic expansivity of the medium :math:`\beta^\star=10^8\;\text{[MPa}^{-1}\text{]}`. Thus, the only active coupling is that of 
...
We will plot the values of temperature by an analytical calculation :math:`P_{an}` and the corresponding umerical values of temperature 
:math:`P_{num}` at the free end of the cantilever w.r.t. time in the left part of 
...
swelling of the cantilever due to pore fluid pressure increase:
...
.. list-table:: 
   ...
   * - .. figure:: _images/Cauchy_3D_Hydroelasticity_Pcalc_Panal.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center

         Evolution of pore fluid pressure (:math:`P\;\left[MPa\right]` at the top free surface of the block w.r.t. time.
         
     - .. figure:: _images/Cauchy_3D_Hydroelasticity_u_anal.svg
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
   
         Evolution of the normal displacement (:math:`u_y\;\left[mm\right]`) at the top of the block with respect to time.

.. raw:: html

        <center>
          <video  width="500" controls>
            <source src="_images/3D_Pressure_diffusion.mp4" type="video/mp4">
           Your browser does not support the video tag.
          </video>
        </center>
      
      Evolution of the pore fluid pressure inside the 3D block.
