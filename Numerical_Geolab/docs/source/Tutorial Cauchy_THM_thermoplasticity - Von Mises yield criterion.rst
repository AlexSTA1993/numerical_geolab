==================================================
Cauchy THM couplings: Thermoplasticity application
==================================================
In this tutorial we will validate the complex behavior of our model when yielding occurs in the presence of thermomechanical couplings.
...
:py:mod:`ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_TM_Thermoplasticity_0`
...
We proceed now with building the main Finite element model, corresponding to shearing of a Cauchy elastoplastic cube with a Von Mises yield criterion and strain hardening.
...
We will provide here commentary for each method used inside the childclass :py:class:`THM3D_FEproblem(UserFEproblem)<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_TM_Thermoplasticity_0.THM3D_FEproblem>`
...
interior of the domain and use of the nodal selection algorithm inside the Numerical Geolab module :py:mod:`feproblem<ngeoFE.feproblem>` (see method :py:meth:`set_history_output_indices()<ngeoFE.feproblem.UserFEobject.set_history_output_indices>`) returns an empty set. The method :py:meth:`set_svars_history_output_indices()<ngeoFE.feproblem.UserFEobject.set_svars_history_output_indices>` needs to be called -internally- instead.
...
We create the :py:class:`Boundary<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_TM_Thermoplasticity_0.Boundary>` child class that will handle the assignment of different labels at the different faces of the unit cube. We implement this by 
...
Inside the :py:class:`Boundary<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_THM_ThermoHydroplasticity_1>` subclass we define the logical test for assigning the specific boundary node in one of the boundary regions, where the boundary conditions will be applied.
...
:py:meth:`set_bcs()<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_TM_Thermoplasticity_0.THM3D_FEproblem.set_bcs>`: 
...
components of the vectorspace ([3],[4]) the type of boundary condition is set to 1 indicating that Neuman conditions that specify the spatial derivative
of the particular vector field components need to be appied. Here, we set the displacement component parallel to the horizontal axis [0].
...
The material properties passed into the material algorithm, are set with the helper method: aaasssss
...
.. code-block:: python
    ...
    def set_material_1_properties(self):
        ...
        lstar=10.**(-8) #thermal expansivity of mixture #No thermal pressurization coupling 
        bstar=10.**8 #hydraulic expansivity of mixture # No hydraulic coupling
...
We intent to verify that our analysis indeed predicts the correct relations between the dissipation and the temperature increase. To do so we will set a minimum value pf yield strength, so that
...
:math:`\Lambda=\frac{\lambda^\star}{\beta^\star}=0` in order to ignore the coupling with the pore fluid pressure equation. We set the themal expansion coefficient :math:`=1\text{[}{^{\text{o}}\text{C}}^{-1}\text{]}` so that we can measure the changes 
...
due to heating in the material stresses of the constrained cube. We also set the hydraulic expansivity of the medium :math:`\beta^\star=10^8\;\text{[MPa}^{-1}\text{]}` to avoid the hydraulic coupling
of the pore fluid pressure equation.   
...
. The full problem description script can be found :py:mod:`here<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_THM_Thermoplasticity_0>`. 
...
.. code-block:: python
    ...
    def run_analysis_procedure(self,reference_data_path):
        saveto=reference_data_path+"THM-RESULTS/THERMO_PLASTIC/test1D_THM3D.xdmf"
        self.problem_step = 0
...
We will plot the Temperature :math:`T` and vertical displacement :math:`u_z` values w.r.t. time in Figure :ref:`Figure 1<my_table>`:
...
.. code-block:: python
            ...
            x1.insert(0,0)
            y1.insert(0,0)We can plot the vertical displacement w.r.t. time in Figure in :ref:`Figure 1<my_table>`, in order to check the degree of confinement:
...
.. list-table:: 
   ...
   * - .. figure:: _images/Cauchy_3D_Thermoplasticity_1_T.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center
     ...
     - .. figure:: _images/Cauchy_3D_Thermoplasticity_1_u.svg
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
         ...
         displacement remains zero, which implies an increase in the normal vertical 
         stress :math:`\sigma_{22}\;\left[MPa\right]`. 

We will also plot the stresses :math:`\sigma_{12},\sigma_{23},\sigma_{12}` values w.r.t. time in Figure :ref:`Figure 2<my_table2>`:
...
.. code-block:: python
            ...
            fig, ax1, ax2, color1, color2 = plotting_params.object_plot_axes('$t$ [s]', y1_txt='$\sigma_{22}\; $[kPa]',color1='',y2_txt='$\sigma_{12}$ [kPa]',color2='', title='',mode='2')
            plotting_params.object_plot(x2, y2, y1, ax1, ax2, mode='2',color1=color1,color2=color2,label_string='')
            plotting_params.show_plot()                       
            
            plotting_params.plot_legends(ngeo_parameters.reference_data_path, fig, filename='Cauchy_3D_Thermoelasticity_sigma122_12',mode='1') 
          
.. _my_table2:

.. list-table:: 

   * - .. figure:: _images/Cauchy_3D_Thermoplasticity_sigma22_12.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center
     ...
     - .. figure:: _images/Cauchy_3D_Thermoplasticity_sigma23_12.svg
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
         ...
         The value of the shear stress :math:`\sigma_{12}\;\left[MPa\right]`, 
         which is the only component present in the yield criterion, stays constant 
         equal the value of the parameter :py:const:`cc`. 
         The measure of the shear effective stress :math:`\sigma_{23}\;\left[MPa\right]`, remains equal to zero according to the 
         associative flow rule used in the analysis.
