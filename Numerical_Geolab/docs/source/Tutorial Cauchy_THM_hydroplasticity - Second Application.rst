===========================================================================
Cauchy THM couplings: Hydroplasticity application - Undrained Triaxial test
===========================================================================
...
We fix all the facets of the cube except the top along the direction parallel to each normal vector respectively. A vertical displacement is then applied in the top facet, 
so that the cube is compressed. The boundaries of the cube are considered to be insulated from its surroundings meaning that no flow of heat 
...
thermomechanical and thermo- hydromachanical couplings of the original system of equations. The presence of the hydro-mechanical coupling is expected to lead 
to a pore fluid pressure increase due to the undrained boundary conditions present.  

The file for this tutorial can be found in: 
:py:mod:`ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_HM_Hydroplasticity_1`
...
We will provide here commentary for each method used inside the childclass :py:class:`THM3D_FEproblem(UserFEproblem)<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_HM_Hydroplasticity_1.THM3D_FEproblem>`
...
and plastic strain rate, the generalized variables include the mechanical stresses and total strains (elastic, plastic decomposition), the rate of plastic strains and the fluxes and partial derivatives of the pressure and temperature fields. 
For a 3D problem twelve generalized strain components (and consequently  twelve generalized stress components) are needed 
...
We create the :py:class:`Boundary<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_HM_Hydroplasticity_1.Boundary>` child class that 
...
Inside the :py:class:`Boundary<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_HM_Hydroplasticity_1>` subclass we define the logical test for assigning the specific boundary node in one of the boundary regions, where the boundary conditions will be applied.
...
by making use of the function :py:meth:`set_initial_conditions()< ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_HM_Hydroplasticity_1.set_initial_conditions>` :
...
:py:meth:`set_bcs()<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_HM_Hydroplasticity_1.THM3D_FEproblem.set_bcs>`: 
    ...
    def set_bcs(self):
        ...
        if self.problem_step == 0:
            ...
            bcs=[
                ...
                [1,[0,[3],0.]],#Undrained boundary conditions
                [2,[0,[3],0.]],
                [1,[0,[4],0.]],#Adiabatic boundary conditions
                [2,[0,[4],0.]],
                ]        
        return bcs  
...
In this example, the region id ranges from 1 to 2 indicating the top and bottom regions of the boundary respectively. 
...
For the pore fluid pressure and temperature component of the vectorspace ([3], [4]) we set the type of boundary condition to 1, with value 0, indicating that 
during the analysis, we apply undrained, adiabatic Neuman conditions.
...
In the next step, we implement an undrained triaxial test. We set the vertical displacement component [2] of the top facet (region_id=1) to a final negative value -0.2, 
while the normal displacemnets of the lateral sides and the bottom of the cube are constrained to zero.
This ensures, that at all stages of the analysis we apply a compressive volumetric strain.
...
method :py:meth:`create_Gauss_point_query_domain<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_THM_ThermoHydroelasticity.THM3D_FEproblem.create_Gauss_point_query_domain()>`
...
:py:meth:`set_material_params()< ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_HM_Hydroplasticity_1.set_material_1_properties>` : 
...
.. code-block:: python
    ...
    def set_material_1_properties(self):
        ...
        """
        EE=2000.;nu=0. ;cc=0.1 ;tanfi=0.5;tanpsi=0.;Hsfi=0.;Hscc=0.
        GG=EE/(2.*(1.+nu))
        KK=EE*GG/(3.*(3.*GG-EE))
        chi=1.0      #permeability;
        eta_f=1.0    #fluid_viscosity
        lstar=0. #thermal expansivity of mixture results in no thermal pressurization coupling 
        bstar=10.**(-1) #hydraulic expansivity of mixture results in no hydraulic coupling
...
excerted by the compressive material pressure. We will set the specific heat :math:`\rho C=10^8\frac{^\text{o}\text{C}}{\text{J}}` to remove the temperature increase due to yielding.
...
pore fluid pressure equation. We set the themal expansion coefficient :math:`\alpha=0\text{[}{^{\text{o}}\text{C}}^{-1}\text{]}` to deactivate the thermomechanical coupling. 
Finally, we set the hydraulic compressibility of the material to :math:`\beta^\star=10^{-1}\;\text{MPa}^{-1}` for easier testing of the numerical results.
...
.. code-block:: python
    ...
    def run_analysis_procedure(self,reference_data_path):
        saveto=reference_data_path+"THM-RESULTS/HYDRO_PLASTIC/Hydroplasticity_2_test3D_THM3D_step_0.xdmf"
        ...
        for i in range(nsteps):
            ...
            self.feobj.initBCs()
            filename = 'THM-RESULTS/HYDRO_PLASTIC/Hydroplasticity_2_test3D_THM3D_step_'+str(i+1)
...
We apply then one increment of compressive triaxial deformation, to provoke yielding of the material and pore fluid pressure increase.
...
.. list-table:: 
   ...
   * - .. figure:: _images/Cauchy_3D_Hydroplasticity_2_P.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center
         ...
         Due to the specimen's confinement, it's compression, leads to lower values of compressive material stresses, and thus 
         the specimen presents a hardening response. 
         
     - .. figure:: _images/Cauchy_3D_Hydroplasticity_2_evol.svg
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
   
         Volumetric strain (:math:`\varepsilon_{v}`) with respect to time, compression is negative. 

We will also plot a comparison of the pore fluid pressure increase and the volumetric decrease during the test.
...
.. list-table:: 
   ...
   * - .. figure:: _images/Cauchy_3D_Hydroplasticity_2_P_evol.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center

         Pore fluid pressure increase w.r.t. volumetric decrease during the analysis.          
