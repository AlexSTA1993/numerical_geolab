==========================================================================================
Cauchy THM couplings: Thermo-hydro plasticity application - Drucker-Prager yield criterion
==========================================================================================
...
Our model is a unit cube, that consists of four three dimensional tetrahedral elements.
The base of the cube is fixed and its top facet is prescribed to move along the normal dimension.  
The boundaries of the cube are considered to be insulated from its surroundings meaning that no flow of heat 
and pore fluid is allowed to take place. Furthermore, all lateral displacements are constrained to zero. 
...
:py:mod:`ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_THM_ThermoHydroplasticity_1`
...
the childclass :py:meth:`THM3D_FEproblem(UserFEproblem)<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_THM_ThermoHydroplasticity_1.THM3D_FEproblem()>`.
...
plastic strain multiplier and generalized plastic strain rate. The generalized variables include the mechanical stresses and total strains 
(elastic, plastic decomposition), the rate of plastic strains and the fluxes and partial derivatives of the pressure and temperature fields. For a 3D problem twelve generalized strain components (and consequently  twelve generalized stress components) are needed 
...
For the pore fluid pressure and temperature components of the vectorspace ([3], [4]) we set the type of boundary condition to 1, with value 0, indicating that 
during the triaxial analysis, we apply undrained, adiabatic Neuman conditions.
...
In the next step, we implement an adiabatic undrained triaxial test. We set the horizontal displacement component [0] of the top facet (region_id=1) to a final negative value -0.2, 
while the normal displacemnets of the top and the bottom of the cube are constrained to zero.
This ensures, that during the analysis the volumetric strain is equal to zero leading to changes in the pore fluid and material pressure respectively.
...
:py:meth:`set_material_params()< ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_THM_ThermoHydroplasticity_1.set_material_1_properties>` : 
...
| Once the analysis material(s) have been set we are able to exit the class definition for :ref:`THM3D_FEproblem<ngeoFE_unittests.Multiphysics.Cauchy_tests.ThreeD.BVP.CAUCHY_DP_THM_ThermoHydroplasticity_1.THM3D FEproblem>`.
...
.. code-block:: python
   :emphasize-lines: 7
    ...
    def run_analysis_procedure(self,reference_data_path):
        saveto=reference_data_path+"THM-RESULTS/THERMO_HYDRO_PLASTIC/Triaxial_test3D_THM3D_initial_0.xdmf"
        self.problem_step = 0
        print('initial')
        ...
        for i in range(nsteps):
            ...
            self.feobj.initBCs()
            import os
            print(os.path.abspath(reference_data_path))
            saveto=reference_data_path+"THM-RESULTS/THERMO_HYDRO_PLASTIC/Triaxial_test3D_THM3D_shearing_"+str(i+1)+".xdmf"
...
We apply then one increment of shear deformation, to provoke yielding of the material and pore fluid pressure increase, due to thermal pressurization.
...
We can plot the vertical displacement w.r.t. time in Figure in :ref:`Figure 1<my_table>`, in order to check the effect of confinement:

.. _my_table:

.. list-table:: 

   * - .. figure:: _images/Cauchy_3D_Thermo_Hydro_plasticity_Triaxial_J_2_P_eff.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center
     ...
     - .. figure:: _images/Cauchy_3D_Triaxial_Thermo_Hydro_plasticity_evol.svg
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
...
We next plot the values of Temperature :math:`T\;\left[^o\text{C}\right]` and pore fluid pressure :math:`P\;\left[MPa\right]` 
...
.. list-table:: 
   ...
   * - .. figure:: _images/Cauchy_3D_Triaxial_Thermo_Hydro_plasticity_T_P.svg
         :height: 200 px
         :width: 400 px
         :alt: alternate text
         :align: center
     ...
     - .. figure:: _images/Cauchy_3D_Triaxial_Thermo_Hydro_plasticity_DT_DP.svg
         :height: 200px
         :width: 400 px
         :alt: alternate text
         :align: center
